...
const getAllLecture = async ({ token, limit = false, faculty = false, page = false, select = false, sort = false }) => {
    ...
    let url = `${apiURL}/lecture/admin/?${faculty}${limit}${page}${select}${sort}`
    return await fetch(url, {
        method: 'GET',
        headers: headers(token)
    }).then(res => res.json())
